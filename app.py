...
.inner-card { background: #0d1117; border: 1px solid #30363d; border-radius: 10px; padding: 14px; margin-bottom: 8px; }
...
@st.cache_data
def load_data():
    DF = pd.read_csv('final_data (3).csv', encoding='UTF-8-SIG')
...
def get_realtime(sido):
    try:
        ...
        params = {'serviceKey': API_KEY, 'returnType': 'json', 'numOfRows': '100', 'pageNo': '1', 'sidoName': sido, 'ver': '1.0'}
        ...
        return df_rt['pm25Value'].mean(), df_rt['pm10Value'].mean(), df_rt['dataTime'].iloc[0]
    except:
...
with hc1:
    st.markdown("<div style='padding:20px 0 10px 0;'><div style='font-size:1.6rem; font-weight:700; color:#e6edf3;'>초미세먼지 예측 대시보드</div></div>", unsafe_allow_html=True)
with hc2:
    selected = st.selectbox('지역 선택', regions, index=list(regions).index('서울') if '서울' in list(regions) else 0)
with hc3:
    st.markdown(f"<div style='text-align:right; padding:24px 0; color:#8b949e; font-size:0.82rem;'>{now.strftime('%Y.%m.%d %H:%M')} 기준</div>", unsafe_allow_html=True)
...
with gr:
    if rt_g in ['나쁨','매우나쁨']:
        ...
        guides = [('#d29922','&#128084;','마스크 권장','민감군은 마스크를 착용하세요'),('#58a6ff','&#127939;','야외활동 주의','장시간 야외활동 자제하세요'),('#2ea043','&#128168;','환기 가능','잠깐씩 환기하세요')]
    else:
        guides = [('#2ea043','&#127774;','외출하기 좋은 날','야외활동을 즐기세요'),('#2ea043','&#128168;','환기 권장','창문을 열어 환기하세요'),('#58a6ff','&#127939;','운동하기 좋은 날','야외 운동을 즐기세요')]
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="sec-title">&#9989; 행동 가이드</div>', unsafe_allow_html=True)
    for c,icon,t,d in guides:
        st.markdown(f"<div class='inner-card' style='display:flex; gap:12px; align-items:center;'><div style='width:38px; height:38px; border-radius:10px; background:{c}22; display:flex; align-items:center; justify-content:center; font-size:1.2rem;'>{icon}</div><div><div style='font-size:0.88rem; font-weight:600; color:#e6edf3;'>{t}</div><div style='font-size:0.8rem; color:#8b949e;'>{d}</div></div></div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

al,ar = st.columns([3,2])
with al:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="sec-title">&#128269; 원인 분석</div>', unsafe_allow_html=True)
    factors = [
        ('&#128168; 풍속', latest['ws_avg']/10*100, f"{latest['ws_avg']:.1f} m/s", '낮을수록 나쁨'),
        ('&#128167; 습도', latest['hm_avg'], f"{latest['hm_avg']:.0f}%", '높을수록 정체'),
        ('&#127783; 기압', (latest['pa_avg']-980)/60*100, f"{latest['pa_avg']:.0f} hPa", '높을수록 정체'),
        ('&#127807; PM2.5', min(current_pm25/80*100,100), f"{current_pm25:.0f} μg/m³", '전날 농도 영향'),
        ('&#127968; 계절', 75 if now.month in [12,1,2,3] else 30, get_season_str(now.month), '겨울/봄 높음'),
    ]
    for name,pct,display,hint in factors:
        pct = min(max(pct,0),100)
        c = '#f85149' if pct>70 else '#f0883e' if pct>40 else '#58a6ff'
        st.markdown(f"<div style='margin-bottom:14px;'><div style='display:flex; justify-content:space-between; margin-bottom:5px;'><span style='font-size:0.85rem; color:#e6edf3;'>{name} <span style='font-size:0.75rem; color:#8b949e;'>{hint}</span></span><span style='font-size:0.85rem; color:{c};'>{display}</span></div><div style='background:#0d1117; border-radius:6px; height:8px;'><div style='width:{pct}%; height:100%; background:{c}; border-radius:6px;'></div></div></div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

with ar:
    lm = df[(df['region_key']==selected)&(df['date'].dt.month==(now.month-1 if now.month>1 else 12))]['PM25_avg'].mean()
    ly = df[(df['region_key']==selected)&(df['date'].dt.year==now.year-1)&(df['date'].dt.month==now.month)]['PM25_avg'].mean()
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="sec-title">&#128197; 과거 대비 비교</div>', unsafe_allow_html=True)
    for lbl,val,diff_v in [('현재 실시간',current_pm25,None),('지난달 평균',lm,current_pm25-lm),('작년 같은달',ly,current_pm25-ly)]:
        diff_html = f"<div style='font-size:0.82rem; color:{'#f85149' if diff_v>0 else '#2ea043'}; margin-top:5px;'>{'▲' if diff_v>0 else '▼'} {abs(diff_v):.0f} μg/m³ {'높음' if diff_v>0 else '낮음'}</div>" if diff_v is not None else ''
        st.markdown(f"<div class='inner-card'><div style='font-size:0.8rem; color:#8b949e;'>{lbl}</div><div style='font-size:1.9rem; font-weight:700; color:{rt_c};'>{val:.0f}<span style='font-size:0.85rem; color:#8b949e;'> μg/m³</span></div>{diff_html}</div>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="sec-title">&#128204; PM2.5 등급 기준</div>', unsafe_allow_html=True)
gc1,gc2,gc3,gc4 = st.columns(4)
for col,g,c,rng,desc,icon in [(gc1,'좋음','#2ea043','0~15 μg/m³','야외활동 자유','&#128994;'),(gc2,'보통','#d29922','16~35 μg/m³','민감군 주의','&#129000;'),(gc3,'나쁨','#f0883e','36~75 μg/m³','마스크 착용','&#128993;'),(gc4,'매우나쁨','#f85149','76+ μg/m³','외출 자제','&#128308;')]:
    with col:
        st.markdown(f"<div style='background:#0d1117; border:1px solid {c}44; border-radius:10px; padding:16px; text-align:center; border-top:3px solid {c};'><div style='font-size:1.6rem;'>{icon}</div><div style='font-size:1rem; font-weight:700; color:{c};'>{g}</div><div style='font-size:0.85rem; color:#e6edf3;'>{rng}</div><div style='font-size:0.78rem; color:#8b949e;'>{desc}</div></div>", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

st.markdown("<div style='text-align:center; color:#8b949e; font-size:0.8rem; padding:20px 0;'>데이터: 에어코리아 + 기상청 &nbsp;|&nbsp; AI: XGBoost &nbsp;|&nbsp; R² Score: 0.91</div>", unsafe_allow_html=True)
